...
def signup():
    try:
        ...
        if not username or not contact or not password:
            return jsonify({'status': 'error', 'message': 'सभी फ़ील्ड भरना जरूरी है!'})
        ...
        # Check duplicate username manually to prevent crashes
        c.execute("SELECT uid FROM users WHERE username=?", (username,))
        if c.fetchone():
            conn.close()
            return jsonify({'status': 'error', 'message': 'यह यूजरनेम पहले से किसी और का है!'})
        ...
        return jsonify({'status': 'error', 'message': f'सर्वर एरर: {str(e)}'})
...
def login():
    try:
        ...
        return jsonify({'status': 'error', 'message': 'सर्वर कनेक्ट नहीं हो पाया'})
